...
#==================================================================
# Flag respondents with all “Don’t know” (now NA)
#==================================================================
gn_flagged <- gn_cleaned %>%
  dplyr::mutate(
    all_dk = dplyr::if_else(
      rowSums(is.na(dplyr::across(dplyr::everything()))) == ncol(gn_cleaned),
      1, 0
    )
  )

# Flag high "Don't Know" proportion

gn_flagged <- gn_flagged %>%
  dplyr::mutate(
    dk_prop = rowMeans(is.na(dplyr::across(starts_with("gn")))),
    dk_50 = ifelse(dk_prop >= 0.5, 1, 0)
  )


# Check Counts
table(gn_flagged$all_dk)
table(gn_flagged$dk_50)


# Sensitivity check (important)
...
# Run your model twice:
# Full sample (minus all DK)
# model_full <- gn_flagged %>%
#   dplyr::filter(all_dk == 0)


# # Restricted sample (drop high DK)
model_restricted <- gn_flagged %>%
  dplyr::filter(all_dk == 0, dk_50 == 0)
...
gn_reversed <- model_full %>%
  dplyr::mutate(
    dplyr::across(
      dplyr::all_of(reverse_items),
      ~ {
        x <- as.numeric(as.character(.x))
        ifelse(!is.na(x), 4 - x, NA)
      }
    )
  )
...
gn_analysis <- as.data.frame(gn_analysis)

sapply(gn_analysis, function(x) length(unique(na.omit(x))))

gn_analysis <- gn_analysis %>%
  dplyr::select(dplyr::starts_with("gn"))
