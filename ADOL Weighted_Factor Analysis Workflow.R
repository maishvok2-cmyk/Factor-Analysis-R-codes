...
# the psych workflow does not directly support probability weights cleanly in the same way survey packages do.
# The best practice is to apply weights when computing the correlation matrix.

# Weighted polychoric correlation
# Use lavaan::lavCor() instead of psych::polychoric() because it allows sampling weights.


# Apply weights in these exact steps:
  # Weighted
      # polychoric matrix
      # EFA extraction
      # CFA estimation
      # descriptive means/percentages of factor scores
  # Not necessary
      # reverse coding
      # missing data cleaning
      # item filtering by variance
      # alpha/omega
      # row mean score construction



# For publication-quality work:
    # EFA on weighted polychoric matrix
    # CFA with sampling.weights
    # report both weighted and unweighted N

# This is especially important for national survey data.

# The standard practice is:
    # EFA/CFA → weighted
    # Alpha/Omega → usually unweighted, but state this in methods

# Report:
# Reliability coefficients were estimated on the analytic sample, 
# while factor extraction and confirmatory models incorporated sampling weights.

# Descriptive statistics were weighted using survey weights
# Factor scores were computed as unweighted means of items


# sampling.weights = "d_pspweight_scaled"
# Use one weight only, not all four.
# Quick rule for choosing weight
    # d_pspweight_scaled → most common for analysis
    # d_popweight_raw → if you need original survey weights
    # d_pspweight_* → if design requires PSU level weighting
...
#==================================================================
# Select GN Items and weight variables
#==================================================================
...
# Combine GN items + weights
gn_df <- df %>% 
  select(all_of(c(gn_cols, weight_var)))

cat("Selected items:", length(gn_cols), "\n")
cat("Weight variables added:", length(weight_var), "\n")
...
# Define final item list (AFTER dropping)

item_names <- setdiff(names(gn_dropped), weight_var)
...
#==================================================================
# Missingness
# Whenever you do transformations, exclude weights:

# Split the weights vars and the gn items

#==================================================================
# FILTER ITEMS ONLY (KEEP WEIGHT SAFE)
#==================================================================
items_df <- gn_reversed %>% select(-all_of(weight_var))
weights_df <- gn_reversed %>% select(all_of(weight_var))

# Missingness
valid_pct <- colMeans(!is.na(items_df))
items_df <- items_df[, valid_pct >= 0.70]

# Variance
items_df <- items_df[, apply(items_df, 2, sd, na.rm = TRUE) > 0.01]

# Merge back weight
gn_analysis <- bind_cols(items_df, weights_df)

cat("Remaining items:", ncol(gn_analysis), "\n")  

# ==================================================================
# # IMpute (Median)-> Removed
# #==================================================================
# gn_analysis <- gn_df4
# 
# # Remove this code:
# # For WLSMV with ordinal items, do not median impute.
# # Because median imputation biases thresholds and polychoric correlations.
# # Let lavaan handle missing data.
# 
...
#==================================================================
weight_var <- "d_pspweight_scaled"
item_names <- setdiff(names(gn_analysis), weight_var)
...
# This gives you a weighted polychoric correlation matrix.
...
# Keep As is: This is the correct point for weights in EFA.
#------------------------------------------------------------------------------
...
# Apply weights when creating factor scores
#----------------------------------------------------------
...
# The code rowMeans(na.rm = TRUE) is okay for individual-level latent proxies, 
# but when reporting descriptive statistics, 
# use weighted summaries.
# construct scores → unweighted row means is okay
# population summaries of scores → weighted

# For Example:
# weighted.mean(df_with_scores$Factor1_Score,
#               w = df_with_scores$weight_var,
#               na.rm = TRUE)

# Weighted SD
# Hmisc::wtd.var(df_with_scores$Factor1_Score,
#                weights = df_with_scores$weight_var,
#                na.rm = TRUE)
#----------------------------------------------------------
...
factor_descriptives <- data.frame(
  ...
  # report both
  N_unweighted = sapply(factor_scores, function(x) sum(!is.na(x))),
  N_weighted = round(sapply(factor_scores, function(x) sum(w[!is.na(x)])), 0),
  
  Mean = sapply(factor_scores, w_mean, w = w),
  SD = sapply(factor_scores, w_sd, w = w),
  Min = sapply(factor_scores, min, na.rm = TRUE),
  Max = sapply(factor_scores, max, na.rm = TRUE),
  Median = sapply(factor_scores, w_median, w = w)
)
...
factor_summary <- factor_descriptives %>%
  ...
  select(Factor, Items, N_unweighted, N_weighted, Mean, SD, Median, Min, Max, Alpha)
...
# This is the main place weights must be used.
weight_var <- "d_pspweight_scaled"

fit <- lavaan::cfa(
  cfa_model,
  data = gn_analysis,
  ordered = setdiff(names(gn_analysis), weight_var),
  estimator = "WLSMV",
  sampling.weights = weight_var
)
...
# Apply weights here:
  # CFA → sampling.weights
  # Descriptives → weighted mean, SD, median

# Do not apply weights here:
    # alpha
    # factor scores
    # preprocessing
...
# Theoretical vs EFA does not change weighting.
# Weights are about representativeness, not how the model was derive
...
group_vars <- c("age_group", "sex_label", "residence")

#----------------------------------------------------------
# UNWEIGHTED N COUNTS
#----------------------------------------------------------
n_age <- df_with_scores %>% count(age_group, name = "n")
n_sex <- df_with_scores %>% count(sex_label, name = "n")
n_res <- df_with_scores %>% count(residence, name = "n")
n_total <- nrow(df_with_scores)



# Helper: build a named list of stat_N = "**level**\nN = n" for modify_header
make_col_headers <- function(n_df, var_col) {
  headers <- paste0("**", n_df[[var_col]], "**\nN = ", format(n_df$n, big.mark = ","))
  names(headers) <- paste0("stat_", seq_along(headers))
  as.list(headers)
}
...
tbl_age <- svy_design |>
  ...
  add_p(test = all_continuous() ~ "svy.wilcox.test") |>
  modify_header(label ~ "**Variable**") |>
  bold_labels()

tbl_age <- inject(modify_header(tbl_age, !!!make_col_headers(n_age, "age_group")))
...
tbl_sex <- svy_design |>
  ...
  add_p(test = all_continuous() ~ "svy.kruskal.test") |>
  modify_header(label ~ "**Variable**") |>
  bold_labels()

tbl_sex <- inject(modify_header(tbl_sex, !!!make_col_headers(n_sex, "sex_label")))
...
tbl_res <- inject(modify_header(tbl_res, !!!make_col_headers(n_res, "residence")))
...
final_table <- tbl_merge(
  ...
  modify_caption(
    "**Reversed and 98 set Missing**"
  ) |>
  modify_footnote(
    all_stat_cols() ~ "Weighted mean (SD). N is unweighted."
  )
...
# Table 4. Factor scores by demographic characteristics (survey weighted estimates)
...
# #----------------------------------------------------------
# # factors_tbl <- tbl_merge(
# #   tbls = list(tbl_overall, tbl_age, tbl_sex, tbl_res),
# #   tab_spanner = c("Overall", "Age (Years)", "Sex", "Residence")
# # ) %>%
# #   bold_labels() %>%
# #   modify_caption(
# #     "**Table 4. Factor scores by demographic characteristics (weighted survey estimates)**"
# #   ) %>%
# #   modify_footnote(
# #     all_stat_cols() ~
# #       "Values are weighted means (SD). n is unweighted sample size."
# #   )
# 
# factors_tbl <- tbl_merge(
#   tbls = list(tbl_overall, tbl_age, tbl_sex, tbl_res),
...
#----------------------------------------------------------
# FACTOR SCORES -> Do not apply weights here.
#----------------------------------------------------------
...
#----------------------------------------------------------
# DESCRIPTIVES -> weighted descriptives
#----------------------------------------------------------
...
# All stats are now populated. The root cause was a weight misalignment: w was pulled from df_with_scores (4230 rows), but factor_scores_cfa has only 3887 rows — those produced by fit_forced using listwise deletion on its 45 items.

# The fix is to define w upfront using the same complete-case mask that fit_forced used:
...
#----------------------------------------------------------
# SAFE WEIGHTED FUNCTIONS
#----------------------------------------------------------


library(dplyr)

# Correctly align weights to the 3887 complete cases used by fit_forced
forced_items <- lavaan::lavNames(fit_forced)
mask_forced  <- complete.cases(gn_analysis[, forced_items])
w <- gn_analysis[[weight_var]][mask_forced]
...
w_sd <- function(x, w) {
  idx <- !is.na(x) & !is.na(w)
  m <- weighted.mean(x[idx], w[idx])
  sqrt(weighted.mean((x[idx] - m)^2, w[idx]))
}
w_median <- function(x, w) {
  idx <- !is.na(x) & !is.na(w)
  Hmisc::wtd.quantile(x[idx], weights = w[idx], probs = 0.5)
}

cfa_descriptives <- data.frame(
  Factor = colnames(factor_scores_cfa),
  
  N_unweighted = sapply(factor_scores_cfa, function(x) sum(!is.na(x))),
  
  N_weighted = round(sapply(factor_scores_cfa, function(x) {
    idx <- !is.na(x) & !is.na(w)
    sum(w[idx])
  }), 0),
  
  Mean   = sapply(factor_scores_cfa, function(x) w_mean(x, w)),
  SD     = sapply(factor_scores_cfa, function(x) w_sd(x, w)),
  Min    = sapply(factor_scores_cfa, function(x) min(x, na.rm = TRUE)),
  Max    = sapply(factor_scores_cfa, function(x) max(x, na.rm = TRUE)),
  Median = sapply(factor_scores_cfa, function(x) w_median(x, w))
) |>
  dplyr::mutate(across(where(is.numeric), ~ round(., 3)))
